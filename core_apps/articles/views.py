...
class ArticleListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        logger.info(
            f"Article {serializer.data.get('title')} created_by {self.request.user.first_name}"
        )


class ArticleRetieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Article.objects.all()
    serializer_class = ArticleSerializer
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        clap.delete()
        return Response(
            {"detail": "Clap removed from article"}, status=status.HTTP_200_OK
        )
