...
@pytest.mark.django_db
def test_get_full_nmae(normal_user):
...
@pytest.mark.django_db
def test_get_short_nmae(normal_user):
...
@pytest.mark.django_db
def test_email_is_incorrect(user_factory):
...
def test_create_user_with_no_first_name(user_factory):
    ...
    assert str(err.value) == "User must have Firstname"
...
def test_create_user_with_no_last_name(user_factory):
    ...
    assert str(err.value) == "User must have lastname"
...
def test_create_user_with_no_email(user_factory):
    ...
    assert str(err.value) == "User must have email"
...
def test_create_superuser_is_not_superuser(user_factory):
    with pytest.raises(ValueError) as err:
        user_factory.create(is_superuser=False,
                            is_staff=True)

    assert str(err.value) == "Superuser must have is_superuser=True"

@pytest.mark.django_db
def test_create_superuser_is_not_active(user_factory):
    with pytest.raises(ValueError) as err:
        user_factory.create(is_active=False,
                            is_superuser=True,
                            is_staff=True)
